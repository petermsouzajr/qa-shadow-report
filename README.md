...
Get your first CSV report without any credentials or setup:
...
2. **Run the setup wizard** (creates config file):
   ```bash
   npx qasr-setup
   ```
   Answer the prompts for your framework (Cypress or Playwright). You can skip the Google Sheets questions for now.

3. **Run your test suite** so test results JSON exists (e.g., `npm test` or your existing test command).
...
5. **Find your report:**
   - The CSV will be in your `downloads/` folder (or the path you configured in `shadowReportConfig.js`)
   - The report includes test results, execution time, status, error details, and any team/category/case-id annotations you've added to your tests

**Optional next step:** Want to send reports to Google Sheets? See [Sheets Setup Guide](#sheets-setup-guide) below.

## Just want to see a demo?

Check out the working examples on the `demo` branch:

```bash
git clone https://github.com/petermsouzajr/qa-shadow-report.git
cd qa-shadow-report
git checkout demo
cd cypress-example    # or playwright-example
npm install
npm test              # Runs tests and generates CSV report
```

Each example has a complete README with test structure and annotation examples. See the [Demo Branch](#demo-branch) section for details.
...
For those who want to see `qa-shadow-report` in action before integrating it into their projects, we have set up a `demo` branch in the repository. This branch includes fully configured example projects where you can run and observe the report generation process.

#### What's in the Demo Branch

The `demo` branch contains:
- **`cypress-example/`** - A complete Cypress project with real test suites
- **`playwright-example/`** - A complete Playwright project with real test suites
- All the package source code from `main`

Both example projects include:
- Current framework versions (Cypress 16.x, Playwright 1.49+)
- Realistic test specs with proper qa-shadow-report annotations
- Full configuration files and documentation
...
2. **Choose your framework:**
   ```bash
   cd cypress-example    # For Cypress demo
   # OR
   cd playwright-example # For Playwright demo
   ```

3. **Install dependencies:**
   
   Each example uses the local package via `file:..` for seamless integration:
   ```bash
   npm install
   ```
   
   For contributors working on the package itself, you can use npm link:
   ```bash
...
5. **Optional: Set up Google Sheets reporting:**
   ...
   Follow the setup instructions in each example's README to configure Google Sheets integration with your own credentials.

#### Demo Branch Maintenance

The `demo` branch is automatically kept in sync with `main` via a GitHub Action that:
- Merges changes from `main` to keep the package source code current
- Preserves demo-specific files and configurations
- Updates the example projects to match the latest versions on `main`

This ensures that the demo always reflects the current state of the package while maintaining working example projects.

#### What You'll See

When you run the demo tests, you'll observe:
- Tests organized by type (API, UI) and annotated with teams, categories, and case IDs
- JSON test results generated by the framework reporters
- qa-shadow-report processing the results
- Report generation (CSV by default, Google Sheets if configured)

Each example's README contains detailed information about the test structure, available scripts, and customization options.
