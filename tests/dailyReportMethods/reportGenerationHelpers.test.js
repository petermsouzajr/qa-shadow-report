...
describe('report generation helpers', () => {
  describe('generatePlaceholders', () => {
    ...
    it('should throw a TypeError if count is not a number', () => {
      // @ts-ignore
      expect(() => generatePlaceholders('3')).toThrow('The count must be a number.');
    });
  
    it('should throw a TypeError if count is not an integer', () => {
      expect(() => generatePlaceholders(3.5)).toThrow('The count must be an integer.');
    });
  
    it('should throw a TypeError if count is a negative number', () => {
      expect(() => generatePlaceholders(-1)).toThrow('The count must be a non-negative integer.');
    });
  ...
  describe('generateReportEntry', () => {
    ...
    it('should throw a TypeError if title is not a string', () => {
      // @ts-ignore
      expect(() => generateReportEntry(123, 'SUM(A1:A10)')).toThrow('Title must be a string.');
    });
  
    it('should throw an Error if title is an empty string', () => {
      expect(() => generateReportEntry('', 'SUM(A1:A10)')).toThrow('Title cannot be an empty string.');
    });
  
    it('should throw a TypeError if formula is not a string', () => {
      // @ts-ignore
      expect(() => generateReportEntry('Test Title', 123)).toThrow('Formula must be a string.');
    });
  
    it('should throw an Error if formula is an empty string', () => {
      expect(() => generateReportEntry('Test Title', '')).toThrow('Formula cannot be an empty string.');
    });
  });
  
  describe('generateStateReports', () => {
    it('should generate report entries with valid inputs', () => {
      const defaultHeaderMetrics = ['# Total tests', '# Passed tests', '# Failed tests'];
      const index = 0;
    ...
    it('should throw a TypeError if defaultHeaderMetrics is not an array', () => {
      // @ts-ignore
      expect(() => generateStateReports('not an array', 0)).toThrow('defaultHeaderMetrics must be an array of strings.');
    });
  
    it('should throw a TypeError if defaultHeaderMetrics contains non-string elements', () => {
      // @ts-ignore
      expect(() => generateStateReports([1, 2, 3], 0)).toThrow('Each item in defaultHeaderMetrics must be a string.');
    });
  
    it('should throw a TypeError if index is not a number', () => {
      // @ts-ignore
      expect(() => generateStateReports(['# Total tests'], 'not a number')).toThrow('index must be an integer.');
    });
  
    it('should throw a RangeError if index is out of bounds', () => {
      expect(() => generateStateReports(['# Total tests'], 1)).toThrow('index is out of bounds for defaultHeaderMetrics.');
    });
  
    it('should throw a TypeError if the extracted type is not a string', () => {
      expect(() => generateStateReports(['#  tests'], 0)).toThrow('The extracted type must be a non-empty string.');
    });
  });
  ...
  describe('generateTeamReport', () => {
    it('should generate a report entry with a valid type', () => {
      ...
      expect(reportEntry).toEqual(['# Team Alpha tests passed', 'Team Alpha formula tests passed']);
    });
  
    it('should throw a TypeError if type is not a string', () => {
      // @ts-ignore
      expect(() => generateTeamReport(123)).toThrow('Type must be a non-empty string.');
    });
  
    it('should throw a TypeError if type is an empty string', () => {
      expect(() => generateTeamReport('')).toThrow('Type must be a non-empty string.');
    });
  ...
  describe('generateTypeReport', () => {
    ...
    it('should throw a TypeError if type is not a string', () => {
      // @ts-ignore
      expect(() => generateTypeReport(123)).toThrow('Type must be a non-empty string.');
    });
  
    it('should throw a TypeError if type is an empty string', () => {
      expect(() => generateTypeReport('')).toThrow('Type must be a non-empty string.');
    });
  ...
  describe('generateReport', () => {
    ...
    it('should throw an error if types is not an array', () => {
      // @ts-ignore
      expect(() => generateReport('not an array', [], 0)).toThrow('Types must be an array.');
    });
  
    it('should throw an error if types contains non-string elements', () => {
      // @ts-ignore
      expect(() => generateReport([123], [], 0)).toThrow('All types must be strings.');
    });
  
    it('should throw an error if payload is not an array', () => {
      // @ts-ignore
      expect(() => generateReport([], 'not an array', 0)).toThrow('Payload must be an array.');
    });
  
    it('should throw an error if searchIndex is not a non-negative integer', () => {
      expect(() => generateReport([], [], -1)).toThrow('SearchIndex must be a non-negative integer.');
    });
  
    it('should throw an error if isTeam is not a boolean', () => {
      // @ts-ignore
      expect(() => generateReport([], [], 0, 'not a boolean')).toThrow('isTeam must be a boolean.');
    });
  ...
  describe('buildFormulas', () => {
    ...
    it('should create formulas correctly', () => {
      const formulas = buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, mockConstants);
      expect(formulas).toBeInstanceOf(Object);
      expect(formulas).toEqual({
        'formula base': '=COUNTIFS(B1:B10, "*testType*")&" ("&ROUND(COUNTIFS(B1:B10, "*testType*")/9 * 100)&"%)"',
        'formula skipped/pending': '=COUNTIFS(B1:B10, "<>*passed*", B1:B10, "<>*failed*")&" ("&ROUND(COUNTIFS(B1:B10, "<>*passed*", B1:B10, "<>*failed*")/9 * 100)&"%)"',
        'formula tests passed': '=COUNTIFS(A1:A10, "*testType*", B1:B10, "passed")&" of "&COUNTIFS(A1:A10, "*testType*")&"  -  "&"("&ROUND(COUNTIFS(A1:A10, "*testType*", B1:B10, "passed")/(COUNTIFS(A1:A10, "*testType*")) * 100)&"%)"',
    ...
    it('throws error for invalid type', () => {
      // @ts-ignore
      expect(() => buildFormulas(123, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, mockConstants)).toThrow('Type must be a string.');
    });
  
    it('throws error for invalid headerRowIndex', () => {
      expect(() => buildFormulas(validType, -1, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, mockConstants)).toThrow('Header row index must be a positive integer.');
      // @ts-ignore
      expect(() => buildFormulas(validType, '1', validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, mockConstants)).toThrow('Header row index must be a positive integer.');
    });
  
    it('throws error for invalid totalNumberOfRows', () => {
      expect(() => buildFormulas(validType, validHeaderRowIndex, 0, validBodyRowCount, validSubjectColumn, validStateColumn, mockConstants)).toThrow('Total number of rows must be an integer greater than header row index.');
      // @ts-ignore
      expect(() => buildFormulas(validType, validHeaderRowIndex, '10', validBodyRowCount, validSubjectColumn, validStateColumn, mockConstants)).toThrow('Total number of rows must be an integer greater than header row index.');
    });
  
    it('throws error for invalid bodyRowCount', () => {
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, 0, validSubjectColumn, validStateColumn, mockConstants)).toThrow('Body row count must be a positive integer.');
      // @ts-ignore
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, '9', validSubjectColumn, validStateColumn, mockConstants)).toThrow('Body row count must be a positive integer.');
    });
  
    it('throws error for invalid subjectColumn', () => {
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, '', validStateColumn, mockConstants)).toThrow('Subject column must be a non-empty string.');
      // @ts-ignore
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, 123, validStateColumn, mockConstants)).toThrow('Subject column must be a non-empty string.');
    });
  
    it('throws error for invalid stateColumn', () => {
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, '', mockConstants)).toThrow('State column must be a non-empty string.');
      // @ts-ignore
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, 123, mockConstants)).toThrow('State column must be a non-empty string.');
    });
  
    it('throws error if FORMULA_TEMPLATES is not an array', () => {
      const invalidConstants = { ...mockConstants, FORMULA_TEMPLATES: 'not an array' };
      // @ts-ignore
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, invalidConstants)).toThrow('FORMULA_TEMPLATES must be a non-empty array.');
    });
  
    it('throws error if FORMULA_KEYS is not an array', () => {
      const invalidConstants = { ...mockConstants, FORMULA_KEYS: 'not an array' };
      // @ts-ignore
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, invalidConstants)).toThrow('FORMULA_KEYS must be a non-empty array.');
    });
  
    it('throws error if FORMULA_TEMPLATES and FORMULA_KEYS have different lengths', () => {
      const invalidConstants = { FORMULA_TEMPLATES: ['template1'], FORMULA_KEYS: ['key1', 'key2'] };
      expect(() => buildFormulas(validType, validHeaderRowIndex, validTotalNumberOfRows, validBodyRowCount, validSubjectColumn, validStateColumn, invalidConstants)).toThrow('FORMULA_TEMPLATES and FORMULA_KEYS arrays must have the same length.');
    });
  });
  ...
  describe('constructHeaderRegex', () => {
    ...
    it('should throw an error if getKeysPattern returns a non-string value', () => {
      ...
      expect(() => constructHeaderRegex(getKeysPattern)).toThrow('The keys pattern must be a non-empty string.');
    });
  ...
  describe('determineSubjectColumn', () => {
    ...
    it('should return the correct column letter for a category type', () => {
      const result = determineSubjectColumn('category1', columnsAvailable, mockNumberToLetter, mockConstants);
      expect(result).toBe('C'); // Assuming numberToLetter works as expected
    });
  
    it('should return the correct column letter for a type', () => {
      const result = determineSubjectColumn('type1', columnsAvailable, mockNumberToLetter, mockConstants);
      expect(result).toBe('B'); // Assuming numberToLetter works as expected
    });
  
    it('should return the correct column letter for a team type', () => {
      const result = determineSubjectColumn('teamType', columnsAvailable, mockNumberToLetter, mockConstants);
      expect(result).toBe('A'); // Assuming numberToLetter works as expected
    });
  
    it('should throw an error if type is not a string', () => {
      // @ts-ignore
      expect(() => determineSubjectColumn(123, columnsAvailable, mockNumberToLetter, mockConstants)).toThrow('Type must be a string.');
    });
  
    it('should throw an error if columnsAvailable is not an array', () => {
      // @ts-ignore
      expect(() => determineSubjectColumn('type1', 'not an array', mockNumberToLetter, mockConstants)).toThrow('columnsAvailable must be an array.');
    });
  
    it('should throw an error if numberToLetter is not a function', () => {
      // @ts-ignore
      expect(() => determineSubjectColumn('type1', columnsAvailable, 'not a function', mockConstants)).toThrow('numberToLetter must be a function.');
    });
    ...
    it('should throw an error if constants do not include required functions', () => {
      ...
      // @ts-ignore
      expect(() => determineSubjectColumn('type1', columnsAvailable, mockNumberToLetter, invalidConstants)).toThrow('Constants must include TEST_CATEGORIES_AVAILABLE and TEST_TYPES_AVAILABLE functions.');
    });
    ...
    it('should throw an error if TEST_TYPES_AVAILABLE does not return an array of strings', () => {
      ...
      expect(() => determineSubjectColumn('type1', columnsAvailable, mockNumberToLetter, invalidConstants)).toThrow('TEST_TYPES_AVAILABLE must return an array of strings.');
    });
    ...
    it('should throw an error if TEST_CATEGORIES_AVAILABLE does not return an array of strings', () => {
      ...
      expect(() => determineSubjectColumn('type1', columnsAvailable, mockNumberToLetter, invalidConstants)).toThrow('TEST_CATEGORIES_AVAILABLE must return an array of strings.');
    });
  });  
...
describe('getKeysPattern', () => {
  ...
  it('should throw an error if FORMULA_KEYS is not an array', () => {
    ...
    // @ts-ignore
    expect(() => getKeysPattern(constants)).toThrow('FORMULA_KEYS must be an array.');
  });

  it('should throw an error if FORMULA_KEYS contains non-string elements', () => {
    const constants = { FORMULA_KEYS: ['key1', 123, 'key3'] };
    // @ts-ignore
    expect(() => getKeysPattern(constants)).toThrow('FORMULA_KEYS must be an array of strings.');
  });
  ...
  it('should throw an error if FORMULA_KEYS is not provided', () => {
    // @ts-ignore
    expect(() => getKeysPattern('')).toThrow('FORMULA_KEYS must be an array.');
  });
});
  ...
  describe('createMergeQueries', () => {
    ...
    it('should create a valid requestBody for batch updating merges', () => {
      const requestBody = createMergeQueries(validData, validHeaderRowIndex, validSheetId);
    ...
    it('should throw an error if data is not a 2D array of strings', () => {
      // @ts-ignore
      expect(() => createMergeQueries('not an array', validHeaderRowIndex, validSheetId)).toThrow('Data must be a 2D array of strings.');
      // @ts-ignore
      expect(() => createMergeQueries([['A1', 123]], validHeaderRowIndex, validSheetId)).toThrow('Data must be a 2D array of strings.');
    });

    it('should throw an error if headerRowIndex is not a non-negative integer', () => {
      expect(() => createMergeQueries(validData, -1, validSheetId)).toThrow('Header row index must be a non-negative integer.');
      // @ts-ignore
      expect(() => createMergeQueries(validData, 'not a number', validSheetId)).toThrow('Header row index must be a non-negative integer.');
    });

    it('should throw an error if sheetId is not a non-negative integer', () => {
      expect(() => createMergeQueries(validData, validHeaderRowIndex, -1)).toThrow('Sheet ID must be a non-negative integer.');
      // @ts-ignore
      expect(() => createMergeQueries(validData, validHeaderRowIndex, 'not a number')).toThrow('Sheet ID must be a non-negative integer.');
    });

    it('should handle edge cases with no merges required', () => {
      const dataWithNoMerges = [['A1', 'B1'], ['A2', 'B2']];
      const requestBody = createMergeQueries(dataWithNoMerges, validHeaderRowIndex, validSheetId);
      expect(requestBody.requests.length).toBe(0);
    });

    it('should handle single column data', () => {
      const singleColumnData = [['A1'], ['A1'], ['A2']];
      const requestBody = createMergeQueries(singleColumnData, validHeaderRowIndex, validSheetId);
    ...
    it('should handle empty data array', () => {
      const emptyData = [];
      const requestBody = createMergeQueries(emptyData, validHeaderRowIndex, validSheetId);
      expect(requestBody.requests.length).toBe(0);
    });
  });
});
